...
- **THEN** the code lives in `infra/cloud/ssh_keys.py`
...
VM. `SSHMachineRepository.disconnect` is a safe no-op when the `node_id` is
absent from `_sessions`, so calling `disconnect(node.node_id)` when
`_connect_to_vm` itself failed (no session registered) is harmless. The success
path is unchanged: on a successful `_setup_vm`, the session stays registered
for orchestrator reuse.

#### Scenario: Setup failure disconnects before VM deletion
- **WHEN** `_setup_vm` raises `CloudSetupError` or a generic `Exception`
