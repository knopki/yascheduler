...
platform-based filtering that returns a new collection. The engine
collection SHALL also support the read-only mapping protocol: `keys()`
returning a list of engine names, `items()` returning a list of
name-engine pairs, iteration yielding engine names, and `len()`
returning the engine count. The collection SHALL be recognized as a
`Mapping` by `isinstance` checks.
...
#### Scenario: keys returns engine names as a list

- **WHEN** `keys()` is called on an engine collection with engines named "a" and "b"
- **THEN** the result is a `list` containing `"a"` and `"b"`

#### Scenario: iteration yields engine names

- **WHEN** the engine collection is iterated
- **THEN** each yielded value is an engine name (`str`)

#### Scenario: length returns engine count

- **WHEN** `len()` is called on an engine collection with two engines
