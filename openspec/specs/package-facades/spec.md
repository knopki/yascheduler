...
The change SHALL NOT modify the existing public API surface of the
`yascheduler` package:

- `yascheduler/__init__.py` exports (`Yascheduler`, `CONFIG_FILE`, `LOG_FILE`, `PID_FILE`, `__version__`) remain unchanged.
- `yascheduler.aiida_plugin` (AiiDA scheduler entrypoint) remains unchanged.
- `yascheduler.client` remains unchanged.
- `yascheduler.compat` remains internal (not public surface).

#### Scenario: Yascheduler symbol unchanged
- **WHEN** a downstream consumer imports `from yascheduler import Yascheduler`
- **THEN** the symbol resolves with the same signature as before the change
