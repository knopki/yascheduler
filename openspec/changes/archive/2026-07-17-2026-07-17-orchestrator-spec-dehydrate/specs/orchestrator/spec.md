...
Provider selection is delegated to `clouds.select_provider`; the orchestrator
SHALL NOT read `clouds.configs` or hold `adapters`/`configs` dicts.

The composition root passes `local_settings=` and `remote_defaults=` keyword
arguments (not a `Config` aggregate); the orchestrator stores them and reads
`list_private_keys_fn` for the connect loop.
...
lookup) and SHALL use `repository.list_connected()`. The consumer SHALL NOT
perform its own SSH teardown — teardown is owned by `deallocate_node`.
...
- **THEN** the consumer SHALL log `node_id`, `hostname` (when present), and the error, and continue to the next queued node without re-raising
...
All connection identity comes from the `Node` itself; `repository.connect`
reads `node.jump_host` / `node.jump_port` / `node.jump_username` directly.
...
The gate SHALL live in the use case, not in `MachineRepository`.
...
failure SHALL NOT trigger disconnecting that session.
