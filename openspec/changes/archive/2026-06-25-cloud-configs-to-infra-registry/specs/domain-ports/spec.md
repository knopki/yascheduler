...
keeping `application → infra` TYPE_CHECKING-only. `CloudConfig` follows the precedent
of `OccupancyConfig` and `TaskExecutionEngine` — structural Protocols for the minimal
surface a consumer needs, satisfied by the concrete class without inheritance.
