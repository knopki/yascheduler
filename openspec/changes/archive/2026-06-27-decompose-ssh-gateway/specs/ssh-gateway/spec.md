...
**Reason:** `infra/ssh/helpers.py` is deleted. Its symbols migrate by
concern: `ADAPTERS`/`_detect_platform`/`_init_paths`/`MAX_SESSIONS`
move to `infra/ssh/platform/` (see `platform-adapters` delta spec);
`MySSHClient`/`DEFAULT_CONN_OPTS`/`_resolve_tunnel` move to
`infra/ssh/repository.py`; the dead duplicate `my_backoff_exc` is
deleted. The single `SSHMachineGateway` class no longer exists; its
responsibilities are split between `SSHMachineRepository` and
`SSHMachineOperations`.

**Migration:**
- `MySSHClient`, `DEFAULT_CONN_OPTS`, `_resolve_tunnel`: import from
  `yascheduler.infra.ssh.repository`.
- `ADAPTERS`, `_detect_platform`, `_init_paths`, `MAX_SESSIONS`:
  import from `yascheduler.infra.ssh.platform` (see the
  `platform-adapters` delta).
- `my_backoff_exc` (helpers copy): deleted — use the canonical copy in
  `yascheduler.infra.ssh.operations.base`.
- `SSHMachineGateway`: replaced by `SSHMachineRepository` (collection)
  and `SSHMachineOperations` (operations); consumers take two ports
  instead of one.