...
`ip` (including tmp rows with `ip=""`), because the allocator counts tmp rows
toward `max_nodes` capacity.

`list_enabled()` SHALL filter `WHERE enabled = TRUE` with **no python
post-filter**.

`list_disabled()` SHALL filter `WHERE enabled = FALSE AND ip <> ''`.
...
`node.node_id.value` as the key parameter alongside the field params (`ip`,
`ncpus`, `enabled`, `cloud`, `username`, `port`). The `ip` field MUST be in the
`SET` clause — the V1 cloud-allocation lifecycle relies on `update` to flip the
tmp row's `ip` from `""` (the NewNode default) to the real VM ip in a single
`UPDATE`; an `update` without `ip` in `SET` would leave cloud nodes unreachable
after daemon restart and excluded from `list_disabled`'s `WHERE ip <> ''`
filter (VM leak).

Node row mapping SHALL map `ip=row["ip"]` unchanged; `""` is a valid `str` and
