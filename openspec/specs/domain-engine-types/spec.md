...
`Engine` SHALL provide `validate_inputs(ctx: TaskContext) -> None` that raises
`MissingInputFileError` if any `input_file` is missing from `ctx.extra`. The
`Engine` value object SHALL NOT import `ConfigParser` or `SectionProxy` and
SHALL NOT carry `from_config_parser_section` or `get_valid_config_parser_fields`
methods.
