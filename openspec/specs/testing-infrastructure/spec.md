...
Defines the testing infrastructure: pytest configuration, directory structure, dependencies, shared fixtures, and initial unit tests.

## Requirements

### Requirement: pytest configuration in pyproject.toml
The project SHALL declare pytest configuration under `[tool.pytest.ini_options]` in `pyproject.toml`, with testpaths and markers.
