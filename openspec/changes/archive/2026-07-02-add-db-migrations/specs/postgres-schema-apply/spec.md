...
- **THEN** `CREATE TABLE IF NOT EXISTS yascheduler_nodes` includes `ip`, `port`, `username`, `ncpus`, `enabled`, `cloud` columns; `CREATE TABLE IF NOT EXISTS yascheduler_tasks` includes `task_id`, `label`, `metadata`, `ip`, `status` columns
