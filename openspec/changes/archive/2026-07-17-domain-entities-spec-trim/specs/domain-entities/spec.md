...
`@dataclass(frozen=True)` wrapping a single field `value: int`. `TaskId`
validates `value > 0` in `__post_init__` (raising `ValueError` otherwise) and
defines `__str__` returning `str(self.value)`. As a frozen dataclass it is
...
`NewTask` carries no identity and no lifecycle methods; it is converted to a
`Task` only by `TaskRepository.insert`, which attaches the generated `task_id`,
the DB-defaulted `status=TO_DO`, and `allocated_node_id=None`.
...
(identity), `label: str`, `engine: str`, `remote_folder: str | None`,
`local_folder: str | None`, `webhook_url: str | None`,
`webhook_custom_params: dict[str, object]`, `error: str | None`,
`extra: dict[str, object]`, `created_at: datetime`, `updated_at: datetime`,
...
`NewTask`; the conversion happens in exactly one place — `TaskRepository.insert`
— which calls `materialize_task` to attach `TaskCreated` to `events`.

`allocated_node_id` is `None` for unallocated tasks (TO_DO with no node bound)
and for tasks whose node was deleted. It is set only by `run(node_id,
remote_folder)` (the `TO_DO→RUNNING` transition).

The lifecycle SHALL be expressed as five atomic transition methods (`run`,
`reject`, `complete`, `fail`, `abandon`) that each validate the source state,
set all fields that change, construct and append the matching `DomainEvent` to
`events`, and return a new `Task` via `replace`. The `events` field is public;
the unit-of-work reads it directly for event collection.

`remote_folder` is `None` on a freshly-inserted TO_DO task; it is set by `run`.
`local_folder` is `None` until `complete` or `fail` sets it from download
results. `error` is `None` until `reject`, `fail`, or `abandon` sets it.
...
(identity), `hostname: str`, `ncpus: int | None`, `enabled: bool`,
`cloud: str | None`, `username: str`, `port: int`, `jump_host: str | None`,
`jump_port: int`, `jump_username: str`, `external_id: str | None`,
`status: NodeStatus`, `created_at: datetime`, `updated_at: datetime`.
...
`node_id` identifies which `Node` this connected machine represents; it is
preserved across `occupy()`/`release()` transitions.

`platform` is the runtime-discovered platform identifier that the
`is_compatible(engine.platforms)` check reads. It does not live on `Node`.

`state` and `free_since` are the runtime-only state of the connected machine;
they SHALL NOT be persisted and SHALL NOT propagate to `Node`.
...
`@dataclass(frozen=True)` wrapping a single field `value: int`. `NodeId`
validates `value > 0` in `__post_init__` (raising `ValueError` otherwise) and
defines `__str__` returning `str(self.value)`. As a frozen dataclass it is
hashable and usable as a dict key.
...
`OTHER = "OTHER"`. `OTHER` is a placeholder for future node lifecycle states.
The enum value `"OTHER"` matches the `TASK_STATUS` convention (enum label ==
name).
