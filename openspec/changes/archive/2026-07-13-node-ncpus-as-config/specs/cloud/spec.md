...
`replace(node, enabled=True, ...)` call that flips `enabled`. The jump values
SHALL be resolved from the matching `CloudConfig` (`prefix == node.cloud`) if it
sets BOTH `jump_host` and `jump_username`; otherwise from `config.remote.jump_host` /
