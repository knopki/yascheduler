...
AllocationTracker`. It SHALL NOT import `SFTPRetryExc`, `SFTPError`, or `backoff`
...
`operations.download_outputs(session, ...)`. The function SHALL receive `(meta_add,
...
- When `permanent_errors` is non-empty OR `transient_errors` is empty (full
  ...
  takes priority and the function finalises with `task.fail()`.
...
- **THEN** permanent takes priority: the task is transitioned via `task.fail(error_details)`, saved, committed, a `TaskFailed` event is recorded, `tracker.discard(task_id)` is called, and the function returns `True`
...
the `allocate_task`, `consume_task`, and `abandon_node` use cases.
