...
# PURPOSE: Integration tests for TaskRowNotFoundError raised by PostgresTaskRepository.save on 0-row UPDATE.
# SCOPE: save() with non-existent task_id raises and does not append to _saved_tasks; save(expected_status=...) raises when the DB status differs from the guard (double-allocation / lost-update).
...
async def test_save_expected_status_rejects_double_allocation(
    ...
    async with uow_factory() as uow:
        ...
        assert isinstance(survivor.state, Running)
        assert survivor.state.allocated_node_id == node_a.node_id
        assert survivor.state.remote_folder == "/remote/race"
