...
`ProcessResult`, `TaskStatus`, `MachineState`:
- `Task` immutability, the transition methods (`run`/`reject`/`complete`/`fail`/`abandon`)
  and guard errors (`TaskNotTodoError`/`TaskNotRunningError`)
- `Task.fail`/`Task.reject` return a Task with `status=DONE` and `error=reason`
...
- `NewTask` has no `task_id`, no `status`, no `allocated_node_id`, no
  `remote_folder`, no `error` (those appear only on `Task`, or are DB-defaulted)
...
`TaskContext` wrapper, no `context=` kwarg). No `TaskContext`/`with_context`/`task.context`
references in tests.
...
- **THEN** none are present (tests use the typed `Task` / `NewTask` fields directly)
...
- **WHEN** `UnsupportedEngineError("gaussian")`, `TaskNotTodoError(TaskId(42))`, `NoCompatibleNodeError(TaskId(1), ["linux"])` are raised and caught
...
the per-section parser functions:

- `parse_config` round-trips `[db]`, `[local]`, `[remote]` sections into
  `PostgresDbConfig`, `LocalSettings`, `RemoteDefaults` with defaults and
  overrides.
- `parse_config` round-trips `[engine.*]` sections into
  `EngineRepository`.
- `parse_config` round-trips `[cloud.*]` sections into
  `Sequence[CloudConfig]` via the `CLOUD_CONFIG_PARSERS` registry,
  including `ConfigCloudAzure`, `ConfigCloudHetzner`,
...
or a local helper, not via direct attribute assignment
...
`PostgresUnitOfWork` using mocked pg8000 connections:
...
- Node repo: `get_by_id`, `get_by_ids`, `list_enabled`, `list_disabled`,
  `insert`, `update`, `enable`, `disable`, `remove`, `count_by_status`
...
- `allocate_task`: unsupported engine → reject + return False;
  free machine found → allocated + return True; no free machine →
  clouds.allocate + return False
- `consume_task`: successful download → complete; download failure →
  fail
...
Tests SHALL verify that `Yascheduler.queue_submit_task_async` submits a task
through the composition root's `make_cli_deps()` factory and does not
instantiate the daemon graph. Specifically,
...
through the `deps_factory`-injected `CLIDeps.uow_factory` and the `query_tasks`
use case, then maps results to the public six-key dict shape.
