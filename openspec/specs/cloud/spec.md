...
`allocate(provider: str) -> NewNode` returns a `NewNode` (pre-persistence — no
DB write inside the adapter; the caller persists via `NodeRepository.insert`). On
VM creation/setup failure it SHALL raise `CloudAllocateError` or `CloudSetupError`
(domain exceptions). `deallocate(cloud: str, ip: str)` deletes the VM via the
named provider's SDK. `select_provider(platforms, current_counts) -> str | None`
...
- **THEN** a VM is created, set up, and a `NewNode` is returned (no DB write inside the adapter)

#### Scenario: Allocate raises on VM creation failure
- **WHEN** `allocate(provider)` is called and VM creation or setup fails
- **THEN** `CloudAllocateError` or `CloudSetupError` is raised; the caller catches and cleans up the tmp-node
...
`_setup_vm` opens connections via `machine_repository.connect(ip)` during cloud
allocation, and `allocate` does not disconnect them on success. Without `stop()`
draining the repository, those connections leak. `disconnect_all` on
`SSHMachineRepository` is idempotent, so calling it from both `clouds.stop()` and
`Orchestrator.stop()` (shared instance per `dependency-injection`) is safe.
...
session for the failed IP before deleting the VM on the setup-failure path. Both
`except` blocks following `_setup_vm` (the `CloudSetupError` handler and the
generic `Exception` handler) SHALL `await self.machine_repository.disconnect(ip_addr)`
BEFORE `await adapter.delete_node(...)`. Without this, a failed allocation would
leak a stale `FREE` session in `_sessions[ip]` pointing at a deleted VM — the
allocator would then pick that session, attempt operations on it, and raise
`asyncssh.misc.ChannelOpenError`, aborting the free-machine loop.

`SSHMachineRepository.disconnect` is a safe no-op when the IP is absent from
`_sessions` (`self._sessions.pop(ip, None)`), so calling `disconnect(ip_addr)`
when `_connect_to_vm` itself failed (no session registered) is harmless. The
success path is unchanged: on a successful `_setup_vm`, the session stays
registered for orchestrator reuse.

#### Scenario: CloudSetupError disconnects before deleting VM
- **WHEN** `_setup_vm` raises `CloudSetupError` after `_connect_to_vm` registered a session in `_sessions[ip]`
- **THEN** the `CloudSetupError` `except` block awaits `machine_repository.disconnect(ip_addr)` BEFORE `await adapter.delete_node(...)`
...
- **THEN** the generic `except Exception` block awaits `machine_repository.disconnect(ip_addr)` BEFORE `await adapter.delete_node(...)` and re-raising as `CloudSetupError`

#### Scenario: No stale session leaks after failed allocation
- **WHEN** two consecutive `allocate` calls both fail at `_setup_vm`
- **THEN** after each failure `disconnect(ip)` is called, `_sessions` contains no stale `FREE` entries for those IPs, and a subsequent `list_free()` returns an empty list

#### Scenario: Success path does not disconnect
- **WHEN** `_setup_vm` returns a `Node` successfully
- **THEN** `allocate` does NOT call `disconnect(ip_addr)`; the session remains registered for orchestrator reuse after the DB row flips to `enabled=TRUE`
