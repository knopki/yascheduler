...
Define the shared argparse helpers for CLI entry points — reusable argument
definitions (`--config`, `--log-level`, `--log-file`) and the `existing_path`
type validator, consumed by all six CLI command entry points and the three
daemon launchers.
