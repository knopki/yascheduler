...
Concurrent `put()` calls SHALL be deduplicated atomically, closing the
...
#### Scenario: Concurrent puts on full queue are deduplicated
- **WHEN** a `UniqueQueue` at `maxsize=1` has two concurrent `put()` coroutines
  for the same item, both suspended in `super().put()` under a full queue
- **THEN** the queue SHALL NOT contain a duplicate — after draining the queue
  completely, exactly one instance of the item is present
