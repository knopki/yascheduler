...
- **Start daemon**: Create the orchestrator via `make_daemon(config)` and start it as a background `asyncio.Task` via `orchestrator.start()`. The test SHALL NOT call `run_daemon` (which registers signal handlers unsuitable for a test loop).
- **Submit jobs**: Submit four tasks by calling the async entrypoint of yasubmit. Each call SHALL pass `["<script_path>", "--config", "<ini_path>"]` where `<script_path>` is a temp file containing `ENGINE=test_shell` and `LABEL=job_N`, with per-job working-directory isolation. The test SHALL capture `task_id` from the entrypoint's stdout.
- **Assert queued**: After all four submissions, read all four tasks via `uowFactory()` and assert each has status `TO_DO`.
- **Add nodes**: Add two nodes by calling the async entrypoint of yasetnode.
- **Wait for completion**: Poll until all four tasks reach `DONE` (timeout ≥ 60s), capturing each task's `RUNNING` snapshot node IP via `uow.nodes.get_by_id(t.allocated_node_id).ip`.
- **Assert per-task state**: For each task, read it via `uow.tasks.get(id)`, resolve its node via `uow.nodes.get_by_id(task.allocated_node_id)`, and assert.
- **Assert completion and outputs**: For each task, assert `status == DONE`, `task.error is None`, `task.local_folder` is set, and the output file `<local_folder>/1.input.out` exists with content matching the per-job `1.input` payload.
- **Assert distribution**: Collect node IPs from all four tasks (via `uow.nodes.get_by_id(t.allocated_node_id).ip`). Assert the set of allocated node IPs equals `{"<ipA>", "<ipB>"}` (both nodes were used) AND that no single node received all four tasks (reject the 0:4 / 4:0 monopoly case — those indicate one node never accepted work).
- **Assert scheduling activity in logs**: Assert the captured `log_records` contain a trace record with block marker `ALLOCATED` for each of the four `task_id`s, and that the structured fields on those records expose `ip` values covering both node IPs. The test SHALL assert on the trace record's structured fields, not on substring matching against the rendered message.
- **Remove nodes (soft)**: Remove both nodes by calling the async entrypoint of yasetnode with `--remove-soft` once per container. This exercises the soft-remove path which, with no RUNNING tasks, takes the `uow.nodes.remove(ip)` branch.
- **Assert nodes removed**: After both soft-remove calls return, assert `uow.nodes.list_all()` returns an empty list.
- **Stop daemon**: In a `finally` block, call `orchestrator.stop()` and `asyncio.wait_for(orch_task, timeout=10)`.

Status assertions SHALL use `yascheduler.domain.TaskStatus`. The task's allocated node IP is read via `uow.nodes.get_by_id(task.allocated_node_id).ip` (the `Task` entity carries `allocated_node_id`, not `allocated_ip`). The local folder is read via `task.local_folder`. The error field is read via `task.error`. No `task.context` references exist in the test.

#### Scenario: Submitted jobs are initially TO_DO before nodes exist
- **WHEN** four jobs are submitted via the async entrypoint of yasubmit before any node is added
...
- **THEN** there is one trace record with block marker `ALLOCATED` for each of the four `task_id`s
- **AND** the structured fields on those records expose `ip` values covering both node IPs
- **AND** the assertion reads the trace record's structured fields directly, not a substring of the rendered message
...
drive the real entrypoint code paths (`make_daemon`, the async entrypoint of yasubmit)
and assert via `uow_factory` — it SHALL NOT bypass the orchestrator, the cloud
provisioner, the SSH layer, or the persistence layer.
...
missing variable; no Hetzner API call is made. The test carries the existing `e2e`
marker (auto-applied for files under `tests/e2e/`); the project SHALL NOT add any new pytest
...
fixtures — it SHALL NOT depend on `ssh_pool`, `uow_factory`, or `log_records` (session-on-function
raises pytest `ScopeMismatch`). The fixture writes a temp INI with `[db]`, `[local]`, `[remote]`,
...
The test scenario SHALL:

- **Start daemon**: `orchestrator = await make_daemon(hetzner_config)`; start it as a background `asyncio.Task` via `orchestrator.start()` (the test SHALL NOT call `run_daemon`).
- **Submit jobs**: submit TWO tasks via the async entrypoint of yasubmit, each in its own temp CWD holding a distinct `1.input` payload, capturing `task_id` from stdout.
- **Assert queued**: both tasks are `TO_DO` before any node exists.
- **Assert autoscale**: poll `uow.nodes.list_all()` until a `cloud == "hetzner"` node row appears; record its IP into `observed_ips`; timeout ≥ 600s.
- **Wait for completion**: poll until both tasks reach `DONE`, capturing each task's `RUNNING` snapshot node IP via `uow.nodes.get_by_id(task.allocated_node_id).ip`; timeout ≥ 600s.
- **Assert outputs**: for each task, assert `status == DONE`, `task.error is None`, `task.local_folder` is set, and `<local_folder>/1.input.out` exists matching the per-job payload.
- **Assert tasks ran on cloud nodes**: each task's allocated node IP (read via `uow.nodes.get_by_id(task.allocated_node_id).ip`) is the IP of some `cloud == "hetzner"` node observed during the test. The test SHALL NOT assert both allocated node IPs are identical (with `max_nodes = 1` the idle-deallocate loop MAY provision a 2nd VM; that race is non-fatal).
- **Assert cloud-path logs**: assert the captured `log_records` contain a trace record with block marker `CLOUD_DONE` whose structured fields expose `ip` matching the provisioned node and `cloud=hetzner`, and a trace record with block marker `CLOUD_DELETE` whose structured fields expose `ip` and `cloud=hetzner` referencing the node. The test SHALL assert on the trace records' structured fields, not on substring matching against the rendered message. The test SHALL NOT assert on the `CREATED <ip>` line or any `[CloudProvisionerImpl]` line (those are on the top-level `"Orchestrator"` logger, invisible to `log_records`).
- **Assert idle deallocation (strong)**: poll `uow.nodes.list_all()` until the `cloud == "hetzner"` node row is gone (timeout ≥ `idle_tolerance + 120`s). THEN poll `find_srv(client, ip)` (Hetzner API) until it returns `None`, proving the billed VM is actually deleted — a separate, explicit assertion (DB-row removal alone is insufficient).
- **Guaranteed cleanup with loud-fail-on-leak**: in a `finally` block, the test SHALL (a) call `orchestrator.stop()` and await the background task best-effort, and (b) for every IP in `observed_ips`, call `hetzner_delete_node`. After each delete attempt, call `find_srv(client, ip)`; if it raised OR still returns the server, the test SHALL `pytest.fail(...)` naming the leaked IP and emit an ERROR log. The project SHALL NOT implement any name-prefix "sweep" of unrelated servers. Cleanup deletion calls SHALL be best-effort across multiple IPs BUT a failure to actually delete a VM MUST surface as a test failure, not be swallowed.
...
- **THEN** the orchestrator provisions exactly one `cloud == "hetzner"` node
...
- **AND** the captured `log_records` contain a trace record with block marker `CLOUD_DELETE` whose structured fields expose `cloud=hetzner` referencing the node IP

#### Scenario: Cloud provisioning-success log is captured via CLOUD_DONE trace record
- **WHEN** the cloud node row is committed after `clouds.allocate` succeeds
- **THEN** the captured `log_records` contain a trace record with block marker `CLOUD_DONE` whose structured fields expose `ip` matching the provisioned node's IP and `cloud=hetzner`
- **AND** the assertion reads the trace record's structured fields directly, not a substring of the rendered message
...
- **WHEN** any assertion in the test fails (or an exception is raised) after a Hetzner VM was created and its IP was recorded in `observed_ips`
