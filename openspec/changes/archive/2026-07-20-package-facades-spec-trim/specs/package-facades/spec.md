...
`yascheduler.shared` is the bottom layer and SHALL NOT import from any
other `yascheduler` layer. Both direct and indirect imports are checked.
...
- `yascheduler.infra` — sole entry point for `application` and composition root to consume adapter symbols.
- `yascheduler.application` — sole entry point for `adapters` and composition root to consume application symbols.
...
- **THEN** it imports via the layer facade
...
`make_daemon`, `make_cli_deps`, `CLIDeps`, `Config`, `CONFIG_FILE`, `LOG_FILE`, `PID_FILE`).

#### Scenario: Entrypoints facade re-exports the public wiring symbols
- **WHEN** a consumer imports `from yascheduler.entrypoints import Yascheduler, make_daemon, make_cli_deps, CLIDeps, Config, CONFIG_FILE, LOG_FILE, PID_FILE`
...
The shim SHALL re-export exactly `Yascheduler`.
...
`exclude_type_checking_imports = true`, and a `layers` contract named
