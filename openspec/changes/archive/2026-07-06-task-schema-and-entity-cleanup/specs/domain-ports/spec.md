...
- **THEN** returns a `list[TaskId]` (not `list[int]`); the caller feeds them directly to `update_status(TaskId, ...)`; the filter is by `allocated_node_id` (the canonical allocation identity), NOT by `ip`
