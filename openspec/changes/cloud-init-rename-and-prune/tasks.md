...
- [ ] 1.1 In `yascheduler/infra/cloud/`, rename the file `cloud_config.py`
  → `cloud_init.py` (use `git mv` to preserve history). Verify the old file
  path no longer exists.
- [ ] 1.2 In the renamed `yascheduler/infra/cloud/cloud_init.py`, change the
...
- [ ] 1.3 In `cloud_init.py`, update the GRACE `MODULE_CONTRACT` block:
...
- [ ] 1.4 In `cloud_init.py`, update the `MODULE_MAP` block: change
  ...
  renderer`.
- [ ] 1.5 In `cloud_init.py`, drop the `from .protocols import PCloudConfig`
...
- [ ] 1.6 In `cloud_init.py`, update the `START_CHANGE_SUMMARY` block: add a
  ...
  `PREVIOUS_CHANGE`.
- [ ] 1.7 Remove the FIXME comment at the top of the file (line 19:
...
- [ ] 2.1 In `yascheduler/infra/cloud/protocols.py`:
  ...
  - update the `START_CHANGE_SUMMARY` `LAST_CHANGE` entry referencing this
    proposal.
- [ ] 2.2 In `yascheduler/infra/cloud/__init__.py`:
  ...
  - in `__all__`, drop `"PCloudConfig"` and rename `"CloudConfig"` →
    `"CloudInitConfig"`.
- [ ] 2.3 In `yascheduler/infra/cloud/manager.py`:
...
- [ ] 2.4 In `yascheduler/infra/cloud/providers/az.py`:
...
- [ ] 2.5 In `yascheduler/infra/cloud/providers/hetzner.py`:
...
- [ ] 2.6 In `yascheduler/infra/cloud/providers/upcloud.py`:
  ...
  - bump `VERSION` and add a `CHANGE_SUMMARY` `LAST_CHANGE` entry.
- [ ] 2.7 In `yascheduler/infra/cloud/providers/vastai.py`:
  - under `TYPE_CHECKING` (line 48), change the import: drop `PCloudConfig`,
    add `CloudInitConfig`.
  ...
  - bump `VERSION` and add a `CHANGE_SUMMARY` `LAST_CHANGE` entry.
- [ ] 2.8 In `tests/unit/test_cloud_provisioner_impl.py`:
...
- [ ] 2.9 Verify no PCloudConfig references remain: run
  `rg -n 'PCloudConfig\b' yascheduler/` → expect zero matches.
- [ ] 2.10 Verify no CloudConfig-renderer references remain in source (only
...
- [ ] 3.1 In `yascheduler/infra/cloud/protocols.py`, delete the entire
  `# FIXME: dead code?` comment (line 93) and the
  `@dataclass(frozen=True) class CloudCapacity:` block (lines 94-100).
- [ ] 3.2 In `yascheduler/infra/cloud/protocols.py`, update the `SCOPE` line
  in the `MODULE_CONTRACT`: drop `CloudCapacity` from the enumerated
  surface.
- [ ] 3.3 In `yascheduler/infra/cloud/protocols.py`, update the `MODULE_MAP`:
  drop the `CloudCapacity - Cloud capacity dataclass` entry.
- [ ] 3.4 In `yascheduler/infra/cloud/protocols.py`, drop the now-unused
...
- [ ] 3.5 In `yascheduler/infra/cloud/__init__.py`, drop `CloudCapacity` from
  the `from .protocols import (...)` block (line 60) and from `__all__`
  (line 72).
- [ ] 3.6 In `yascheduler/infra/cloud/__init__.py`, update the
  `MODULE_MAP` line `CloudCapacity - Cloud capacity dataclass` →
  (delete it).
- [ ] 3.7 Verify no CloudCapacity references remain: run
...
- [ ] 5.1 In `docs/knowledge-graph.xml`, rename the `<M-CLOUD-CONFIG ...>`
...
- [ ] 5.2 In `docs/knowledge-graph.xml`, update the `<M-CLOUD-PROTOCOLS>`
...
- [ ] 5.3 Verify no references to the old `M-CLOUD-CONFIG` (singular) tag
  ...
  → expect zero matches.
- [ ] 5.4 Verify `M-CLOUD-CONFIGS` (plural) is unchanged: run
...
- [ ] 6.1 Run `openspec validate cloud-init-rename-and-prune --json` →
...
- [ ] 6.2 Run `uv run pytest -m unit` → confirm the full unit suite passes,
...
- [ ] 6.3 Run `uv run zuban check` → confirm zero type errors. If a
...
- [ ] 6.4 Run `uv run ruff check .` and `uv run ruff format --check .` →
  confirm clean.
- [ ] 6.5 Run `uv run lint-imports` → confirm the "Clean architecture
...
- [ ] 6.6 Run the final debt-presence greps:
  - `rg -n 'PCloudConfig\b' yascheduler/` → expect zero matches.
...
- [ ] 6.7 Run `openspec validate --all --json` → confirm `valid: true`
  across all changes and main specs.
- [ ] 6.8 Run `python3 scripts/grace_check.py` → confirm exit 0 (XML +
