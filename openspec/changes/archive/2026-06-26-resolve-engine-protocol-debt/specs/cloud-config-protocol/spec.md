...
`CloudConfig` is a structural Protocol for the minimal surface a consumer needs,
satisfied by the concrete `ConfigCloud*` DTOs without inheritance. It stays in
place because there are multiple DTO implementers (`ConfigCloudAzure`,
`ConfigCloudHetzner`, `ConfigCloudUpcloud`, `ConfigCloudVastAI`), in contrast to
the single-implementer engine case where the `OccupancyConfig` and
`TaskExecutionEngine` Protocols were removed (per the `engine-to-domain-frozen`
D4 rationale that a single-implementer Protocol mirroring a concrete class is
duplication whose cost exceeds the Interface-Segregation benefit).
